 ...
 * shared singleton export. A connection blocked on BRPOP cannot carry any other
 * command — see the comment in src/worker/index.ts. A process that needs to both
 * block on the queue and do anything else needs two connections, so the code has
...
export function createRedis(name: string, log: Logger): Redis {
  const redis = new Redis(REDIS_URL, {
     ...
     *
     * That default is wrong for this project. `BRPOP key 0` is supposed to block
    ...
    /** Reconnect with a short backoff instead of hammering a dead server. */
